...
app.post('/json', async (req, res, next) => {
  try {
    ...
    const userPrompt = ` You are an assistant that can only responds in JSON. Do not respond with any text or response, just JSON. Include "id", "title", "artist", "album" in your response. An example response is: [{"id": 1,"title": "Hey Jude","artist": "The Beatles","album": "The Beatles (White Album)","duration": "4:56"}]. Create a list of ${req.body.length} ${req.body.prompt} "${req.body.spotifyData}". `;

    const namePrompt = `You are an assistant that can only responds in JSON. Create a unique & cheeky playlist name in under 5 words based on the following prompt: ${req.body.prompt}. An example response is '{ "playlistName": "playlist name"}'`
