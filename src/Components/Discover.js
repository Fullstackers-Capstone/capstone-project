...
import { getPlaylistById, getAnyUserProfile } from '../../server/api/spotify';
                      ...
                      { playlist.spotData.data.tracks.items.slice(0, 7).map((_track, index) => {
                        return(
                          <div key={ _track.track.duration_ms } className='track-lineitem'>
                            <span style={{ color: '#777777', fontSize: '.75rem', marginRight: '.35rem' }}>
                              { (index + 1) }. 
                            </span>
                  ...
                  <div className='pl-thumb-prompt-container'>
                    <div className='pl-prompt'>
                      <div className='pl-thumb-prompt-content'>
                        <span className='prompt-title' style={{ color: 'gold' }}>
                          Prompt:
                        </span> 
                    ...
                    <div className='pl-thumb-user-container'>
                      <div className='pl-thumb-user-name-container'>
                        <div className='disc-thumb-user-name'>
                          <a href={ `https://open.spotify.com/user/${ playlist.spotData.data.owner.id }` } target='_blank' title='Open in Spotify'>
                            { playlist.spotData.data.owner.display_name.toUpperCase() }
                          </a>
                        </div>
                      </div>
